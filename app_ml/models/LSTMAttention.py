...
class LSTMAttention():
    ...
    def __init__(self, input_size, output_size, optimizer= Adam(learning_rate=0.001)):
        ...
        self.model.add(layers.Attention())
        self.model.add(layers.LSTM(units=64, activation='tanh', return_sequences=True))
        self.model.add(layers.Attention())
        self.model.add(layers.LSTM(units=32, activation='tanh', return_sequences=False))
