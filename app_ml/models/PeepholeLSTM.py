...
class PeepholeLSTM():
    ...
    def __init__(self, input_size, output_size, optimizer= Adam(learning_rate=0.001)):
        ...
        peep_cell = tfa.rnn.PeepholeLSTMCell(128)
        self.model.add(
            layers.RNN(
                peep_cell,
        ...
        peep_cell = tfa.rnn.PeepholeLSTMCell(64)
        self.model.add(layers.RNN(peep_cell, return_sequences=True))
        # peep_cell = keras.experimental.PeepholeLSTMCell(32)
        peep_cell = tfa.rnn.PeepholeLSTMCell(32)
        self.model.add(layers.RNN(peep_cell, return_sequences=False))
